...
public class TestCarResponse {
    ...
    @Schema(description = "재고 상태", example = "대여 가능", allowableValues = {"대여 가능", "재고 없음"})
    ...
    public static TestCarResponse from(Car car) {
        final long stockCount = car.getCarStocks().stream().filter(carStock -> carStock.getStatus() == StockStatus.AVAILABLE).count();
