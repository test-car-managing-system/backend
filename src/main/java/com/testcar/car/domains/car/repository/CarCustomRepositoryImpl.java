...
public class CarCustomRepositoryImpl implements CarCustomRepository, BaseQueryDslRepository {
    ...
    public Optional<Car> findWithStocksById(Long id) {
        final Car result =
                jpaQueryFactory.selectFrom(car)
                        .leftJoin(car.carStocks, carStock).fetchJoin()
    ...
    @Override
    public Page<Car> findAllWithStocksPageByCondition(CarFilterCondition condition, Pageable pageable) {
        final List<Long> coveringIndex =
                jpaQueryFactory
                        .select(car.id)
                        .from(car)
