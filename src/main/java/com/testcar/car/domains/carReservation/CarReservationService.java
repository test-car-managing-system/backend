...
public class CarReservationService {
    ...
    public List<CarReservation> findAllByMemberAndIds(Member member, List<Long> ids) {
        final List<CarReservation> carReservations = carReservationRepository.findAllWithCarStockByIdInAndMemberId(
                ids, member.getId());
    ...
    /** 시험차량을 반납합니다. */
    public List<CarReservation> returnCarReservation(Member member, ReturnCarReservationRequest request) {
        final List<CarReservation> carReservations = this.findAllByMemberAndIds(member, request.getCarReservationIds());
        carReservations.forEach(CarReservation::updateReturn);

        final List<CarStock> carStocks = carReservations.stream().map(CarReservation::getCarStock).toList();
